...
class Scheduler:
    def __init__(self, _PARAMS, _OUTPUT_PROTO):
        self._params = _PARAMS.value
        self._proto = _OUTPUT_PROTO.value
    ...
    def solve():
        ...
        if self._proto:
            print('Writing proto to %s' % self._proto)
            with open(self._proto, 'w') as text_file:
                text_file.write(str(model))

        # Solve the model.
        solver = cp_model.CpSolver()
        if self._params:
            text_format.Parse(self._params, solver.parameters)
...
def main(_):
    const s = Scheduler(_PARAMS, _OUTPUT_PROTO)
    s.solve()
